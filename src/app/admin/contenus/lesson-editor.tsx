  ...
  return (
    ...
    >
      ...
      <SheetContent className="overflow-y-auto">
        <SheetHeader>
          <SheetTitle>{isEdit ? "Modifier la leçon" : "Nouvelle leçon"}</SheetTitle>
          <SheetDescription>
        ...
        <form action={formAction} className="flex flex-col gap-4 px-4">
          ...
          {state.success && !isEdit && currentLessonId && (
            <p className="text-sm text-emerald">
              Leçon créée. Ajoute une vidéo et des documents ci-dessous.
            </p>
          )}
            ...
            {!canUpload ? (
              ...
              <div className="space-y-2">
                {lesson?.resources.map((res) => (
                  <div key={res.id} className="flex items-center gap-2 rounded-lg border border-border bg-muted/30 px-3 py-2 text-xs">
                ...
                <Input ref={resourceTitleRef} placeholder="Titre du document (ex: Checklist ménage)" className="w-full" />
                <label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-border px-3 py-3 text-xs text-muted-foreground hover:bg-muted/30">
