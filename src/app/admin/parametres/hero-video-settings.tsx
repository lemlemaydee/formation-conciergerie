...
import { setHeroVideoUrl, setHeroPosterUrl, removeHeroVideo } from "@/app/admin/parametres/actions";
...
export function HeroVideoSettings({
  ...
  async function handleVideoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setError(null);
    try {
      const url = await uploadToStorage("site-media", "hero", file);
      await setHeroVideoUrl(url);
      setVideoUrl(url);
    } catch (err) {
  ...
  async function handlePosterUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setError(null);
    try {
      const url = await uploadToStorage("site-media", "hero-poster", file);
      await setHeroPosterUrl(url);
      setPosterUrl(url);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Échec de l'upload.");
    } finally {
    ...
    <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
      ...
      {videoUrl && (
        <button
          type="button"
          onClick={async () => {
            if (!confirm("Retirer la vidéo de présentation ? La landing page reviendra à l'aperçu par défaut.")) return;
